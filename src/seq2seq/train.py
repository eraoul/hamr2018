...
VEC_LENGTH_Y = max(NUM_MIDI_CLASSES, NUM_DURATION_CLASSES) + 1  # TODO: should we add 1 for the start command?
VEC_LENGTH_X = VEC_LENGTH_Y - 1 if not ADD_START else VEC_LENGTH_Y  # X values don't have the extra "start" command option
...
def example_generator(train=True):
    ...
    source, target = tokenize_data(folder)

    while True:
        for s, t in zip(source, target):
            yield s.expand_dims(0), t.expand_dims(0)
...
decoder_inputs = Input(shape=(None, VEC_LENGTH_Y), name='decoder_input')
...
def seq2seq(input_seq):
    ...
    target_seq = np.zeros((1, VEC_LENGTH_Y))
